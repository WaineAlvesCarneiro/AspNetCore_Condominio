...
namespace AspNetCore_Condominio.Application.Tests.Features.Moradores.Commands.CreateMorador;

public class CreateMoradorCommandHandlerTests
{
    ...
    private readonly CreateMoradorCommandHandler _handler;
    ...
    public CreateMoradorCommandHandlerTests()
    {
        ...
        _moradorRepoMock.Setup(repo => repo.CreateAsync(It.IsAny<Morador>()))
            .Callback<Morador>(morador => morador.Id = 5)
            .Returns(Task.CompletedTask);
    }

    private CreateMoradorCommand GetValidCommand() => new()
    {
        Nome = "Maria Teste",
        Celular = "21991234567",
        ...
        DataInclusao = DateOnly.FromDateTime(DateTime.Today),
    ...
    public async Task Handle_ComandoValidoEImovelExiste_DeveCriarMoradorEPublicarEvento()
    {
        ...
        _moradorRepoMock.Verify(repo => repo.CreateAsync(
            It.Is<Morador>(m => m.Nome == command.Nome)
        ), Times.Once);

        _mediatorMock.Verify(m => m.Publish(
            It.Is<MoradorCriadoEvent>(e => e.Morador.Id == 5),
    ...
    public async Task Handle_ImovelInexistente_DeveRetornarFalhaENaoCriarMorador()
    {
        ...
        _moradorRepoMock.Verify(repo => repo.CreateAsync(It.IsAny<Morador>()), Times.Never);
        _mediatorMock.Verify(m => m.Publish(It.IsAny<MoradorCriadoEvent>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Fact]
    public async Task Handle_DeveMapearDataEntradaEInclusaoCorretamente()
    {
        var dataEntrada = new DateOnly(2023, 10, 5);
        var command = GetValidCommand();
        command.DataEntrada = dataEntrada;
        ...
        Assert.Equal(DateOnly.FromDateTime(DateTime.Today).Year, moradorCapturado.DataInclusao.Year);
    }
}